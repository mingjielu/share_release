        ...
        *,
        ...
        if self.checkpoint_core_attention and self.training:
            ...
            if inference_context is None or inference_context.is_static_batching():
                original_head_dim = value.shape[-1]
                padded_head_dim = 192
                pad_part = torch.zeros((value.shape[0],value.shape[1],value.shape[2], padded_head_dim - original_head_dim)).to(value.dtype).to(value.device)
                pad_part = pad_part.detach()
                value_padded = torch.cat([value, pad_part], dim=-1)
                ...
                out_padded = core_attn_out.reshape(value.shape[0],value.shape[1],value.shape[2], -1)
                out_padded = out_padded[:,:,:, :original_head_dim]
                core_attn_out = out_padded.reshape(value.shape[0],value.shape[1],-1)
